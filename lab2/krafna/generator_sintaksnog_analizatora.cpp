...
int findItemSet(const set< LRItem > &needle) {
  ...
  if ((lrit = lrItems.find(needle)) != lrItems.end()) {
    return distance(lrItems.begin(), lrit);
  } else {
    return -1;
  }
}
...
void initActionTable(void) {
  ...
  for (lrit = lrItems.begin(); lrit != lrItems.end(); ++lrit, ++i) {
    for (sit = lrit->begin(); sit != lrit->end(); ++sit) {
      ...
      if (A != newNonTerminal && sit->isCompleteItem()) {
        akcija nova;
      ...
      } else if (isTerminal(a) && (j = findItemSet(GOTO(*lrit, a))) != -1) {
        akcija nova;
        nova.setType(POMAK);
        nova.setState(j);
