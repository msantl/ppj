...
// STACK SPACE
int STACK_SPACE_INT = 100;
string STACK_SPACE = "100";

// inkrement
stack< string > INKREMENT;
stack< string > DEKREMENT;

// petlje
stack< string > PETLJA_BREAK;
stack< string > PETLJA_CONTINUE;
...
// radni djelokrug varijabli
vector< string > LOCAL_SCOPE;

// konstante
map< string, string > CONST;

// generira labelu za if i petlje
string generate_label() {
  static int cnt;
  return "LABEL_" + int2string(cnt++);
}

// uveca sve varijable kraj kojih se pojavio postfix ++ operator
void resolve_postfix_inc() {
  while (INKREMENT.empty() == false) {
    string varijabla = INKREMENT.top();

    if (isGlobalVar(varijabla) == true) {
      string label_name = "G_" + toUpper(varijabla);
      appendToFile("\t\t\tLOAD R3, (" + label_name + "); ucitamo varijablu u R3\n");
      appendToFile("\t\t\tADD R3, 1, R3\n");
      appendToFile("\t\t\tSTORE R3, (" + label_name + "); spremimo varijablu u R3\n");
    } else {
      // ako je lokalno definirana varijabla

      int i = -1, j = -1;
      for (int id = 0; id < (int)LOCAL_SCOPE.size(); ++id) {
        if (LOCAL_SCOPE[id] == "") {
          j = id;
        } else if (LOCAL_SCOPE[id] == varijabla) {
          i = id;
        }
      }
      ...
      if (i > j) {
        offset = int2string((i - j) * 4);
      } else {
        offset = int2string(STACK_SPACE_INT + (LOCAL_SCOPE.size() - i) * 4);
      }

      appendToFile("\t\t\tLOAD R3, (R0+" + offset + "); uzmi vrijednost " + varijabla + " sa stoga\n");
      appendToFile("\t\t\tADD R3, 1, R3\n");
      appendToFile("\t\t\tSTORE R3, (R0+" + offset + "); spremimo u " + varijabla + "\n");
    }
...
// umanji sve varijable kraj kojih se pojavio postfix -- operator
void resolve_postfix_dec() {
  while (DEKREMENT.empty() == false) {
    string varijabla = DEKREMENT.top();

    if (isGlobalVar(varijabla) == true) {
      string label_name = "G_" + toUpper(varijabla);
      appendToFile("\t\t\tLOAD R3, (" + label_name + "); ucitamo varijablu u R3\n");
      appendToFile("\t\t\tSUB R3, 1, R3\n");
      appendToFile("\t\t\tSTORE R3, (" + label_name + "); spremimo varijablu u R3\n");
    } else {
      // ako je lokalno definirana varijabla
...
bool implicitnaPromjena(const tip* U, const tip *V) {
  ...
  if (U == tip_const_char::instance() && V == tip_int::instance()) { return true; }
  ...
  if (U == tip_niz::instance(tip_int::instance()) &&
      V == tip_niz::instance(tip_const_int::instance())
  ) {
    return true;
  }

  if (U == tip_niz::instance(tip_const_int::instance()) &&
      V == tip_niz::instance(tip_int::instance())
  ) {
    return true;
  }

  if (U == tip_niz::instance(tip_char::instance()) &&
      V == tip_niz::instance(tip_const_char::instance())
  ) {
    return true;
  }
...
bool eksplicitnaPromjena(const tip* U, const tip *V) {
  ...
  if (U == tip_const_int::instance() && V == tip_char::instance()) { return true; }

  return implicitnaPromjena(U, V);
}
...
const tip* have_types::get_types(int x) {
  return types[x];
}
...
void primarni_izraz__IDN::provjeri(void) {
  ...
  while (vrh != NULL) {
    if (vrh->oznaka_lokalnog_djelokruga == true) {
      globalna = false;
      break;
    }
    vrh = vrh->parent;
  ...
  if (curr->tablica[name].status != 2) {
    // inace ako je funkcija
    string function_name = "F_" + toUpper(name);

    IDN.push(function_name);

  } else if (globalna) {
    // ako je globalno definirana varijabla
    string label_name = "G_" + toUpper(name);

    appendToFile("\t\t\tLOAD R5, (" + label_name + "); ucitamo varijablu u R5\n");
    IDN.push(label_name);
    ...
    appendToFile("\t\t\tLOAD R5, (R0+" + offset + "); uzmi vrijednost "+name+" sa stoga\n");
    IDN.push(name);
  }
...
void primarni_izraz__BROJ::provjeri(void) {
  ...
  if (checkInt(children[0]->prod) == 0) {
    ispis_pogreske();
  }

  long long int value = string2int(vrijednost(children[0]->prod));

  if (UNARNI_OP.empty() == false) {
    switch (UNARNI_OP.top()) {
      case 1:
        value = +value;
        break;
      case 2:
  ...
  string broj = int2string(value);
  string label_name = generate_label();

  if (CONST.find(broj) == CONST.end()) {
    appendToConstFile(label_name + "\t\t\tDW " + broj + "\n");

    CONST[broj] = label_name;
  } else {
    label_name = CONST[broj];
  }

  IDN.push(broj);
  appendToFile("\t\t\tLOAD R5, (" + label_name + "); ucitamo konstantu u R5\n");
...
void primarni_izraz__ZNAK::provjeri(void) {
  ...
  if (checkChar(children[0]->prod) == 0) {
    ispis_pogreske();
  }

  char value = strip_signs(vrijednost(children[0]->prod))[0];
...
void primarni_izraz__NIZ_ZNAKOVA::provjeri(void) {
  ...
  if (checkCharArray(children[0]->prod) == 0) {
    ispis_pogreske();
  }
...
void postfiks_izraz__postfiks_izraz_L_ZAGRADA_D_ZAGRADA::provjeri(void) {
  ...
  appendToFile("\t\t\t; poziv funkcije bez parametara\n");
  appendToFile("\t\t\tCALL " + IDN.top() + "\n"); IDN.pop();
  appendToFile("\t\t\tMOVE R6, R5; spremi povratnu vrijednost u R5\n");
...
void postfiks_izraz__postfiks_izraz_L_ZAGRADA_lista_argumenata_D_ZAGRADA::provjeri(void) {
  ...
  //    arg-tip ~ param~tip
  children[0]->provjeri();
  string function_name = IDN.top(); IDN.pop();
  children[2]->provjeri();

  vector< const tip* > dobiveni = children[2]->get_types();
  pair< const tip*, vector< const tip* > > funkcija = lookup[children[0]->get_type()];

  bool greska = (dobiveni.size() != funkcija.second.size());

  for (int i = 0; greska == false && i < (int)dobiveni.size(); ++i) {
  ...
  string args = int2string(funkcija.second.size() * 4) ;

  appendToFile("\t\t\t; poziv funkcije s parametarima\n");

  appendToFile("\t\t\tCALL " + function_name + "\n");
  appendToFile("\t\t\tADD R7, " + args + ", R7; makni argumente sa stoga\n");
  appendToFile("\t\t\tMOVE R6, R5; spremi povratnu vrijednost u R5\n");
...
void postfiks_izraz__postfiks_izraz_OP_INC::provjeri(void) {
  ...
  string varijabla = IDN.top();
  appendToFile("\t\t\t; postfiks inkrement " + varijabla + "\n");

  INKREMENT.push(varijabla);
...
void postfiks_izraz__postfiks_izraz_OP_DEC::provjeri(void) {
  ...
  string varijabla = IDN.top();
  appendToFile("\t\t\t; postfiks dekrement " + varijabla + "\n");

  DEKREMENT.push(varijabla);
...
void lista_argumenata__izraz_pridruzivanja::provjeri(void) {
  ...
  appendToFile("\t\t\t; dodavanje argumenta na stog\n");
  appendToFile("\t\t\tPUSH R5; stavi je na stog\n");

  types = vector< const tip* >();
...
void unarni_izraz__OP_INC_unarni_izraz::provjeri(void) {
  ...
  children[1]->provjeri();
  if (children[1]->get_l_value() != 1 ||
      implicitnaPromjena(children[1]->get_type(), tip_int::instance()) == 0
  ) {
    ispis_pogreske();
  }

  appendToFile("\t\t\t; prefiks inkrement\n");
  ...
  if (isGlobalVar(varijabla) == true) {
    string label_name = "G_" + toUpper(varijabla);
    appendToFile("\t\t\tLOAD R5, (" + label_name + "); ucitamo varijablu u R5\n");
    appendToFile("\t\t\tADD R5, 1, R5\n");
    appendToFile("\t\t\tSTORE R5, (" + label_name + "); spremimo varijablu u R5\n");
  } else {
    // ako je lokalno definirana varijabla
    ...
    appendToFile("\t\t\tLOAD R5, (R0+" + offset + "); uzmi vrijednost " + varijabla + " sa stoga\n");
    appendToFile("\t\t\tADD R5, 1, R5\n");
    appendToFile("\t\t\tSTORE R5, (R0+" + offset + "); spremimo u " + varijabla + "\n");
  }
...
void unarni_izraz__OP_DEC_unarni_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\t; prefiks dekrement\n");
...
void unarni_izraz__unarni_operator_cast_izraz::provjeri(void) {
  ...
  children[0]->provjeri();
  children[1]->provjeri();
  if (implicitnaPromjena(children[1]->get_type(), tip_int::instance()) == 0) {
    ispis_pogreske();
  }
...
void multiplikativni_izraz__multiplikativni_izraz_OP_PUTA_cast_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\t; dodavanje argumenta na stog za mnozenje\n");
  appendToFile("\t\t\t PUSH R5\n");
  ...
  appendToFile("\t\t\t; dodavanje drugog argumenta na stog za mnozenje\n");
  appendToFile("\t\t\tPUSH R5\n");
  appendToFile("\t\t\tCALL MNOZI\n");
  appendToFile("\t\t\tADD R7, 8, R7; bacanje argumenata sa stoga\n");
  appendToFile("\t\t\tMOVE R6, R5\n");
...
void multiplikativni_izraz__multiplikativni_izraz_OP_DIJELI_cast_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\t; dodavanje argumenta na stog za dijeljenje\n");
  appendToFile("\t\t\t PUSH R5\n");
  ...
  appendToFile("\t\t\t; dodavanje drugog argumenta na stog za dijeljenje\n");
  appendToFile("\t\t\tPUSH R5\n");
  appendToFile("\t\t\tCALL DIJELI\n");
  appendToFile("\t\t\tADD R7, 8, R7; bacanje argumenata sa stoga\n");
  appendToFile("\t\t\tMOVE R6, R5\n");
...
void multiplikativni_izraz__multiplikativni_izraz_OP_MOD_cast_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\t; dodavanje argumenta na stog za modulo\n");
  appendToFile("\t\t\t PUSH R5\n");
  ...
  appendToFile("\t\t\t; dodavanje drugog argumenta na stog za modulo\n");
  appendToFile("\t\t\tPUSH R5\n");
  appendToFile("\t\t\tCALL MODULO\n");
  appendToFile("\t\t\tADD R7, 8, R7; bacanje argumenata sa stoga\n");
  appendToFile("\t\t\tMOVE R6, R5\n");
...
void aditivni_izraz__aditivni_izraz_PLUS_multiplikativni_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\tPUSH R5; spremi prvi operand na stog\n");
  ...
  appendToFile("\t\t\tSTORE R4, (KAKAC); spremi vrijednost R4\n");
  appendToFile("\t\t\tPOP R4\n");
  appendToFile("\t\t\tADD R4, R5, R5\n");
  appendToFile("\t\t\tLOAD R4, (KAKAC)\n");
...
void odnosni_izraz__odnosni_izraz_OP_LT_aditivni_izraz::provjeri(void) {
  ...
  string labelaTrue = generate_label();
  string labelaKraj = generate_label();

  appendToFile("\t\t\tSTORE R4, (KAKAC); spremi vrijednost R4\n");
  appendToFile("\t\t\tPOP R4\n");
  appendToFile("\t\t\tCMP R4, R5\n");
  appendToFile("\t\t\tLOAD R4, (KAKAC)\n");

  appendToFile("\t\t\tJR_SLT " + labelaTrue + "\n");
  appendToFile("\t\t\tMOVE 0, R5\n");
  appendToFile("\t\t\tJR " + labelaKraj + "\n");
  appendToFile(labelaTrue + "\t\t\tMOVE 1, R5\n");
  appendToFile(labelaKraj + "\t\t\t; gotova usporedba LT\n");
...
void odnosni_izraz__odnosni_izraz_OP_GT_aditivni_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\tJR_SGT " + labelaTrue + "\n");
  appendToFile("\t\t\tMOVE 0, R5\n");
  appendToFile("\t\t\tJR " + labelaKraj + "\n");
  appendToFile(labelaTrue + "\t\t\tMOVE 1, R5\n");
  appendToFile(labelaKraj + "\t\t\t; gotova usporedba GT\n");
...
void odnosni_izraz__odnosni_izraz_OP_LTE_aditivni_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\tJR_SLE " + labelaTrue + "\n");
  appendToFile("\t\t\tMOVE 0, R5\n");
  appendToFile("\t\t\tJR " + labelaKraj + "\n");
  appendToFile(labelaTrue + "\t\t\tMOVE 1, R5\n");
  appendToFile(labelaKraj + "\t\t\t; gotova usporedba LTE\n");
...
void odnosni_izraz__odnosni_izraz_OP_GTE_aditivni_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\tJR_SGE " + labelaTrue + "\n");
  appendToFile("\t\t\tMOVE 0, R5\n");
  appendToFile("\t\t\tJR " + labelaKraj + "\n");
  appendToFile(labelaTrue + "\t\t\tMOVE 1, R5\n");
  appendToFile(labelaKraj + "\t\t\t; gotova usporedba GTE\n");
...
void jednakosni_izraz__jednakosni_izraz_OP_EQ_odnosni_izraz::provjeri(void) {
  ...
  appendToFile("\t\t\tPOP R4\n");
  appendToFile("\t\t\tCMP R4, R5\n");
  appendToFile("\t\t\tJR_EQ " + labelaTrue + "\n");
  appendToFile("\t\t\tMOVE 0, R5\n");
  appendToFile("\t\t\tJR " + labelaKraj + "\n");
  appendToFile(labelaTrue + "\t\t\tMOVE 1, R5\n");
  appendToFile(labelaKraj + "\t\t\tLOAD R4, (KAKAC)\n");
...
// pretvori konstante logickog izraza u bool

void genererate_boolean() {
  string labelaTrue = generate_label();
  string labelaKraj = generate_label();

  appendToFile("\t\t\t;pretvori u bool\n");

  appendToFile("\t\t\tCMP R5, 0; usporedi s 0\n");
  appendToFile("\t\t\tJR_NE " + labelaTrue + "\n");
  appendToFile("\t\t\tMOVE 0, R5; slucaj logicke nule\n");
  appendToFile("\t\t\tJR " + labelaKraj + "\n");
  appendToFile(labelaTrue + "\t\t\tMOVE 1, R5; slucaj logicke jedinice\n");

  appendToFile(labelaKraj + "\t\t\t;pretvorba u bool zavrsena\n");
...
void log_i_izraz__log_i_izraz_OP_I_bin_ili_izraz::provjeri(void) {
  ...
  string labelaJump = generate_label();

  genererate_boolean();

  appendToFile("\t\t\tCMP R5, 0\n");
  appendToFile("\t\t\tJR_EQ " + labelaJump + "\n");
  ...
  appendToFile(labelaJump + "\t\t\t; ako je prvi operand lazan ne gledamo dalje\n");
...
void log_ili_izraz__log_ili_izraz_OP_ILI_log_i_izraz::provjeri(void) {
  ...
  appendToFile(labelaJump + "\t\t\t; ako je prvi operand istinit ne gledamo dalje\n");
...
void izraz_pridruzivanja__postfiks_izraz_OP_PRIDRUZI_izraz_pridruzivanja::provjeri(void) {
  ...
  children[0]->provjeri();
  string destination = IDN.top(); IDN.pop();

  if (children[0]->get_l_value() != 1) {
    ispis_pogreske();
  }
  children[2]->provjeri();
  string source = IDN.top(); IDN.pop();
  ...
  if (children[0]->get_type() == tip_niz::instance(tip_int::instance()) ||
      ...
      children[0]->get_type() == tip_niz::instance(tip_char::instance()) ||
      children[0]->get_type() == tip_niz::instance(tip_const_char::instance()) ||
      implicitnaPromjena(children[2]->get_type(), children[0]->get_type()) == 0
  ) {
    ispis_pogreske();
  }

  if (isGlobalVar(destination) == true) {
    appendToFile("\t\t\tSTORE R5, (" + destination + "); spremi vrijednost " + destination +"\n");
  } else {
    // ako je lokalno definirana varijabla
    int i = -1, j = -1;
    for (int id = 0; id < (int)LOCAL_SCOPE.size(); ++id) {
      if (LOCAL_SCOPE[id] == "") {
        j = id;
      } else if (LOCAL_SCOPE[id] == destination) {
        i = id;
      }
    }
    ...
    appendToFile("\t\t\tSTORE R5, (R0+" + offset + "); spremi vrijednost " + destination +"\n");
  }
...
void naredba_grananja__KR_IF_L_ZAGRADA_izraz_D_ZAGRADA_naredba::provjeri(void) {
  ...
  appendToFile(labelaKraj + "\t\t\t;zavrsena naredba grananja\n");
...
void naredba_grananja__KR_IF_L_ZAGRADA_izraz_D_ZAGRADA_naredba_KR_ELSE_naredba::provjeri(void) {
  ...
  string labelaKraj = generate_label();
  string labelaFalse = generate_label();

  appendToFile("\t\t\tCMP R5, 0; usporedi s 0\n");
  appendToFile("\t\t\tJR_EQ " + labelaFalse + "\n");
  ...
  appendToFile(labelaFalse + "\t\t\t;else dio\n");
  children[6]->provjeri();
...
void naredba_petlje__KR_WHILE_L_ZAGRADA_izraz_D_ZAGRADA_naredba::provjeri(void) {
  ...
  string labelaKraj = generate_label();
  string labelaPetlja = generate_label();

  PETLJA_CONTINUE.push(labelaPetlja);
  PETLJA_BREAK.push(labelaKraj);

  appendToFile(labelaPetlja + "\t\t\t; WHILE PETLJA\n");
  children[2]->provjeri();
  ...
  children[4]->provjeri();
  appendToFile("\t\t\tJR " + labelaPetlja + "\n");
  appendToFile(labelaKraj + "\t\t\t; kraj WHILE petlje\n");
...
void naredba_petlje__KR_FOR_L_ZAGRADA_izraz_naredba_izraz_naredba_D_ZAGRADA_naredba::provjeri(void) {
  ...
  appendToFile("\t\t\t; pocetak FOR petlje\n");
  appendToFile("\t\t\t; inicijalizacija\n");
  children[2]->provjeri();
  appendToFile(labelaPetlja + "\t\t\t; pocetak petlje\n");
  children[3]->provjeri();
  ...
  appendToFile("\t\t\tJR " + labelaPetlja + "\n");
  appendToFile(labelaKraj + "\t\t\t; kraj FOR petlje\n");
...
void naredba_petlje__KR_FOR_L_ZAGRADA_izraz_naredba_izraz_naredba_izraz_D_ZAGRADA_naredba::provjeri(void) {
  ...
  string labelaKraj = generate_label();
  string labelaPetlja = generate_label();
  string labelaIzraz = generate_label();

  PETLJA_CONTINUE.push(labelaIzraz);
  PETLJA_BREAK.push(labelaKraj);
  ...
  children[4]->provjeri();
  appendToFile(labelaIzraz + "\t\t\t; izraz FOR petlje");
  children[6]->provjeri();
...
void naredba_skoka__KR_CONTINUE_TOCKAZAREZ::provjeri(void) {
  ...
  appendToFile("\t\t\tJR " + PETLJA_CONTINUE.top() + "\n");
...
void naredba_skoka__KR_BREAK_TOCKAZAREZ::provjeri(void) {
  ...
  appendToFile("\t\t\tJR " + PETLJA_BREAK.top() + "\n");
...
void naredba_skoka__KR_RETURN_TOCKAZAREZ::provjeri(void) {
  ...
  appendToFile("\t\t\tPOP R0; vrati R0\n");
  appendToFile("\t\t\tADD R7, " + STACK_SPACE + ", R7; mjesto za lokalne varijable\n");
  appendToFile("\t\t\tRET; povratak iz funkcije s parametrima\n");
...
void naredba_skoka__KR_RETURN_izraz_TOCKAZAREZ::provjeri(void) {
  ...
  children[1]->provjeri();
  // rezultat izraza je spremljen u R5
  appendToFile("\t\t\tMOVE R5, R6; izrazi se zapisuju u R5\n");
...
void definicija_funkcije__ime_tipa_IDN_L_ZAGRADA_KR_VOID_D_ZAGRADA_slozena_naredba::provjeri(void) {
  ...
  while (curr != NULL) {
    if (curr->tablica.find(name) != curr->tablica.end()) {
      if (curr->tablica[name].status == 0 || curr->tablica[name].status == 2) {
        nasao = true;
        break;
  ...
  appendToFile(function_name + "\t\t\t; definicija funkcije bez parametara\n");
  appendToFile("\t\t\tSUB R7, " + STACK_SPACE + ", R7; mjesto za lokalne varijable\n");
  appendToFile("\t\t\tPUSH R0; upamti R0\n");
  appendToFile("\t\t\tMOVE R7, R0; pomocni registar da nam R7 ne smeta\n");

  LOCAL_SCOPE = vector< string >();
  LOCAL_SCOPE.push_back("");
  // dalje u taj vektor ubacujemo lokalne varijable

  children[5]->provjeri();

  appendToFile("\t\t\tPOP R0; vrati R0\n");
  appendToFile("\t\t\tADD R7, " + STACK_SPACE + ", R7; mjesto za lokalne varijable\n");
  appendToFile("\t\t\tRET; povratak iz funkcije bez parametara\n");
}
void definicija_funkcije__ime_tipa_IDN_L_ZAGRADA_lista_parametara_D_ZAGRADA_slozena_naredba::provjeri(void) {
  ...
  for (int i = 0; i < (int)imena.size(); ++i) {
    bool promjena = true;
    if (tipovi[i] == tip_const_int::instance() || tipovi[i] == tip_const_char::instance()) {
      promjena = false;
    }

    children[5]->tablica[imena[i]] = lex(tipovi[i], promjena, 2);
  }

  string function_name = "F_" + toUpper(name);

  appendToFile(function_name + "\t\t\t; definicija funkcije s parametarima\n");
  appendToFile("\t\t\tSUB R7, " + STACK_SPACE + ", R7; mjesto za lokalne varijable\n");
  appendToFile("\t\t\tPUSH R0; upamti R0\n");
  appendToFile("\t\t\tMOVE R7, R0; pomocni registar da nam R7 ne smeta\n");

  LOCAL_SCOPE = imena;
  // dodamo povratnu adresu
  LOCAL_SCOPE.push_back("");
  // dalje u taj vektor ubacujemo lokalne varijable
...
void init_deklarator__izravni_deklarator_OP_PRIDRUZI_inicijalizator::provjeri(const tip* ntip) {
  ...
  children[2]->provjeri();
  string source = IDN.top(); IDN.pop();
  appendToFile("\t\t\t; dodjeljivanje vrijednosti varijabli\n");


  if (isGlobalVar(destination) == true) {
    appendToConstFile(destination + "\t\t\tDW " + source + "\n");
  } else {

    if (CONST.find(source) == CONST.end()) {
      appendToFile("\t\t\tMOVE R5, R3; spremi vrijednost lokalne varijable\n");
    } else {
      appendToFile("\t\t\tLOAD R3, (" + CONST[source] + "); spremi vrijednost lokalne varijable\n");
    }

    LOCAL_SCOPE.push_back(destination);

    int i = 0;
    for (i = 0; i < (int)LOCAL_SCOPE.size(); ++i) {
      if (LOCAL_SCOPE[i] == "") {
        break;
      }
    }
    string offset = int2string((LOCAL_SCOPE.size() - i - 1) * 4);

    appendToFile("\t\t\tSTORE R3, (R0+" + offset + "); spremi vrijednist " + destination +"\n");
  }
  ...
  if (children[0]->get_type() == tip_int::instance() ||
    ...
    if (children[0]->get_type() == tip_int::instance() ||
      ...
      cmp = tip_int::instance();
    } else {
      cmp = tip_char::instance();
    }

    if (implicitnaPromjena(children[2]->get_type(), cmp) == 0) {
      ispis_pogreske();
    }
    ...
    for (int i = 0; i < children[2]->get_count(); ++i) {
      if (implicitnaPromjena(children[2]->get_types(i), cmp) == 0) {
        ispis_pogreske();
      }
    }
  } else {
...
void izravni_deklarator__IDN::provjeri(const tip* ntip) {
  ...
  while (curr != NULL) {
    ...
    vrh = curr;
    if (curr->oznaka_lokalnog_djelokruga == true) break;
    curr = curr->parent;
  }
  ...
  while (curr != NULL) {
    if (curr->oznaka_lokalnog_djelokruga == true) {
      globalna = false;
      break;
    }
    curr = curr->parent;
  ...
  if (globalna) {
    // ako je globalna definicija
    string label_name = "G_" + toUpper(name);

    IDN.push(label_name);

  } else {
    // inace je lokalna definicija
    IDN.push(name);
  }

  bool promjena = true;
  if (ntip == tip_const_int::instance() || ntip == tip_const_char::instance()) {
    promjena = false;
  }
  type = ntip;
  vrh->tablica[name] = lex(ntip, promjena, 2);
}
...
void izravni_deklarator__IDN_L_ZAGRADA_KR_VOID_D_ZAGRADA::provjeri(const tip* ntip) {
  ...
  while (curr != NULL) {
    ...
    vrh = curr;
    if (curr->oznaka_lokalnog_djelokruga || curr->oznaka_globalnog_djelokruga) break;
    curr = curr->parent;
  }
  ...
  if (postoji == false)vrh->tablica[name] = lex(funkcija, 0, 1);
...
void inicijalizator__izraz_pridruzivanja::provjeri(void) {
  ...
  if (uniformni_znak(curr->prod) == "NIZ_ZNAKOVA") {
    types = vector< const tip* >();
    string niz_znakova = vrijednost(curr->prod);
    count = vrijednost(curr->prod).length() - 1;

    for (int i = 0; i < (int)niz_znakova.size(); ++i) {
      if (niz_znakova[i] == '\\' && i + 1 < (int)niz_znakova.size()) {
        --count;
        ++i;
      }
    }
...
void inicijalizator__L_VIT_ZAGRADA_lista_izraza_pridruzivanja_D_VIT_ZAGRADA::provjeri(void) {
...
// IZGLED POGRESAKA
// primarni izraz
void primarni_izraz__IDN::ispis_pogreske(void) {
  printf(
...
void primarni_izraz__BROJ::ispis_pogreske(void) {
  printf(
...
void primarni_izraz__ZNAK::ispis_pogreske(void) {
  printf(
...
void primarni_izraz__NIZ_ZNAKOVA::ispis_pogreske(void) {
  printf(
...
// postfiks izraz
void postfiks_izraz__postfiks_izraz_L_UGL_ZAGRADA_izraz_D_UGL_ZAGRADA::ispis_pogreske(void) {
  printf(
...
void postfiks_izraz__postfiks_izraz_L_ZAGRADA_D_ZAGRADA::ispis_pogreske(void) {
  printf(
...
void postfiks_izraz__postfiks_izraz_L_ZAGRADA_lista_argumenata_D_ZAGRADA::ispis_pogreske(void) {
  printf(
...
void postfiks_izraz__postfiks_izraz_OP_INC::ispis_pogreske(void) {
  printf(
...
void postfiks_izraz__postfiks_izraz_OP_DEC::ispis_pogreske(void) {
  printf(
...
// unarni izraz
void unarni_izraz__OP_INC_unarni_izraz::ispis_pogreske(void) {
  printf(
...
void unarni_izraz__OP_DEC_unarni_izraz::ispis_pogreske(void) {
  printf(
...
void unarni_izraz__unarni_operator_cast_izraz::ispis_pogreske(void) {
  printf(
...
// cast izraz
void cast_izraz__unarni_izraz::ispis_pogreske(void) {
  // tip <- unarni_izraz.tip
  // l_value <- unarni_izraz.l_value
  // 1. ispis_pogreske(<uanrni_izraz>)
  children[0]->ispis_pogreske();

  type = children[0]->get_type();
  l_value = children[0]->get_l_value();
}
void cast_izraz__L_ZAGRADA_ime_tipa_D_ZAGRADA_cast_izraz::ispis_pogreske(void) {
  printf(
...
// ime tipa
void ime_tipa__KR_CONST_specifikator_tipa::ispis_pogreske(void) {
  printf(
...
// multiplikativni izraz
void multiplikativni_izraz__multiplikativni_izraz_OP_PUTA_cast_izraz::ispis_pogreske(void) {
  printf(
...
void multiplikativni_izraz__multiplikativni_izraz_OP_DIJELI_cast_izraz::ispis_pogreske(void) {
  printf(
...
void multiplikativni_izraz__multiplikativni_izraz_OP_MOD_cast_izraz::ispis_pogreske(void) {
  printf(
...
// aditivni izraz
void aditivni_izraz__aditivni_izraz_PLUS_multiplikativni_izraz::ispis_pogreske(void) {
  printf(
...
void aditivni_izraz__aditivni_izraz_MINUS_multiplikativni_izraz::ispis_pogreske(void) {
  printf(
...
// odnosni izraz
void odnosni_izraz__odnosni_izraz_OP_LT_aditivni_izraz::ispis_pogreske(void) {
  printf(
...
void odnosni_izraz__odnosni_izraz_OP_GT_aditivni_izraz::ispis_pogreske(void) {
  printf(
...
void odnosni_izraz__odnosni_izraz_OP_LTE_aditivni_izraz::ispis_pogreske(void) {
  printf(
...
void odnosni_izraz__odnosni_izraz_OP_GTE_aditivni_izraz::ispis_pogreske(void) {
  printf(
...
// jednakosni izraz
void jednakosni_izraz__jednakosni_izraz_OP_EQ_odnosni_izraz::ispis_pogreske(void) {
  printf(
...
void jednakosni_izraz__jednakosni_izraz_OP_NEQ_odnosni_izraz::ispis_pogreske(void) {
  printf(
...
// bin i izraz
void bin_i_izraz__bin_i_izraz_OP_BIN_I_jednakosni_izraz::ispis_pogreske(void) {
  printf(
...
// bin xili izraz
void bin_xili_izraz__bin_xili_izraz_OP_BIN_XILI_bin_i_izraz::ispis_pogreske(void) {
  printf(
...
// bin ili izraz
void bin_ili_izraz__bin_ili_izraz_OP_BIN_ILI_bin_xili_izraz::ispis_pogreske(void) {
  printf(
...
// log i izraz
void log_i_izraz__log_i_izraz_OP_I_bin_ili_izraz::ispis_pogreske(void) {
  printf(
...
// log ili izraz
void log_ili_izraz__log_ili_izraz_OP_ILI_log_i_izraz::ispis_pogreske(void) {
  printf(
...
// izraz_pridruzivanja
void izraz_pridruzivanja__postfiks_izraz_OP_PRIDRUZI_izraz_pridruzivanja::ispis_pogreske(void) {
  printf(
...
// naredba grananja
void naredba_grananja__KR_IF_L_ZAGRADA_izraz_D_ZAGRADA_naredba::ispis_pogreske(void) {
  printf(
...
void naredba_grananja__KR_IF_L_ZAGRADA_izraz_D_ZAGRADA_naredba_KR_ELSE_naredba::ispis_pogreske(void) {
  printf(
...
// naredba petlja
void naredba_petlje__KR_WHILE_L_ZAGRADA_izraz_D_ZAGRADA_naredba::ispis_pogreske(void) {
  printf(
...
void naredba_petlje__KR_FOR_L_ZAGRADA_izraz_naredba_izraz_naredba_D_ZAGRADA_naredba::ispis_pogreske(void) {
  printf(
...
void naredba_petlje__KR_FOR_L_ZAGRADA_izraz_naredba_izraz_naredba_izraz_D_ZAGRADA_naredba::ispis_pogreske(void) {
  printf(
...
// naredba skoka
void naredba_skoka__KR_CONTINUE_TOCKAZAREZ::ispis_pogreske(void) {
  printf(
...
void naredba_skoka__KR_BREAK_TOCKAZAREZ::ispis_pogreske(void) {
  printf(
...
void naredba_skoka__KR_RETURN_TOCKAZAREZ::ispis_pogreske(void) {
  printf(
...
void naredba_skoka__KR_RETURN_izraz_TOCKAZAREZ::ispis_pogreske(void) {
  printf(
...
// definicija funkcije
void definicija_funkcije__ime_tipa_IDN_L_ZAGRADA_KR_VOID_D_ZAGRADA_slozena_naredba::ispis_pogreske(void) {
  printf(
...
void definicija_funkcije__ime_tipa_IDN_L_ZAGRADA_lista_parametara_D_ZAGRADA_slozena_naredba::ispis_pogreske(void) {
  printf(
...
// lista parametara
void lista_parametara__lista_parametara_ZAREZ_deklaracija_parametra::ispis_pogreske(void) {
  printf(
...
// deklaracija parametara
void deklaracija_parametra__ime_tipa_IDN::ispis_pogreske(void) {
  printf(
...
void deklaracija_parametra__ime_tipa_IDN_L_UGL_ZAGRADA_D_UGL_ZAGRADA::ispis_pogreske(void) {
  printf(
...
// init deklarator
void init_deklarator__izravni_deklarator::ispis_pogreske() {
  printf(
    "<init_deklarator> ::= <izravni_deklarator>\n"
  );
  exit(1);
}
void init_deklarator__izravni_deklarator_OP_PRIDRUZI_inicijalizator::ispis_pogreske() {
  printf(
...
// izravni deklarator
void izravni_deklarator__IDN::ispis_pogreske() {
  printf(
...
void izravni_deklarator__IDN_L_UGL_ZAGRADA_BROJ_D_UGL_ZAGRADA::ispis_pogreske() {
  printf(
...
void izravni_deklarator__IDN_L_ZAGRADA_KR_VOID_D_ZAGRADA::ispis_pogreske() {
  printf(
...
void izravni_deklarator__IDN_L_ZAGRADA_lista_parametara_D_ZAGRADA::ispis_pogreske() {
  printf(
...
g_tree* produkcija(string a) {
  ...
  } else if (a == "inicijalizator__L_VIT_ZAGRADA_lista_izraza_pridruzivanja_D_VIT_ZAGRADA") {
    return new inicijalizator__L_VIT_ZAGRADA_lista_izraza_pridruzivanja_D_VIT_ZAGRADA;
