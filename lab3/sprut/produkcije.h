...
class g_tree {
  public:
    ...
    bool inside_loop;
    bool oznaka_lokalnog_djelokurga;
    ...
    virtual const tip* get_type() { return NULL; }
    virtual bool get_l_value() { return false; }
    virtual int get_count() { return 0; }
    virtual std::string get_name() { return ""; }
    virtual std::vector< const tip* > get_types() { return std::vector< const tip* >(); }
    virtual std::vector< std::string > get_names() { return std::vector< std::string >(); }

    ~g_tree ();
    g_tree() {
      inside_loop = false;
      inside_function = false;
      oznaka_lokalnog_djelokurga = false;
...
class slozena_naredba : public g_tree {
  public:
    slozena_naredba() {
      oznaka_lokalnog_djelokurga = true;
    }
};
...
class inicijalizator__L_VIT_ZAGRADA_lista_izraza_pridruzivanja_D_VIT_ZAGRDA : public inicijalizator {
	public:
		void provjeri(void);
};
